...
import {
  ...
  createAutoEditSourceUploadUrl,
} from "@/lib/shotform-auto-edit-job-store"

/** POST — 브라우저가 소스 MP4를 Supabase에 직접 올리기 위한 signed URL */
export async function POST(req: NextRequest) {
  try {
    ...
    const signed = await createAutoEditSourceUploadUrl(jobId, videoId)
    if (!signed) {
      return NextResponse.json(
        { error: "업로드 URL을 만들지 못했습니다. SUPABASE_SERVICE_ROLE_KEY를 확인해 주세요." },
        { status: 500 }
      )
    }
