...
function buildStarterCmd(origin: string): string {
  ...
  const lines = [
    ...
    // run-agent: 시작할 때마다 최신 portable agent 다운로드 (구버전 미리듣기 422 방지)
    ...
    `echo curl.exe -L --fail -o "%AGENT%" "${agentUrl}" 2^>nul>> "%RUNNER%"`,
    `echo if errorlevel 1 powershell -NoProfile -ExecutionPolicy Bypass -Command "Invoke-WebRequest -UseBasicParsing -Uri '${agentUrl}' -OutFile '%AGENT%'">> "%RUNNER%"`,
