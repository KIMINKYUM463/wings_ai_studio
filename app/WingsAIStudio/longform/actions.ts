...
export async function generateTopics(
  ...
  const categoryPrompts: Record<string, string> = {
    ...
    northkorea: "북한 관련 이야기, 북한의 실상, 탈북자 이야기, 북한 사회와 문화, 북한 역사와 관련된 롱폼 비디오 주제",
    romance_of_three_kingdoms: "삼국지 이야기, 삼국지 인물, 삼국지 전투와 전략, 삼국지 명장면과 관련된 롱폼 비디오 주제",
    folktale: "옛날이야기, 민담, 전래동화, 설화, 구전 이야기와 관련된 롱폼 비디오 주제",
    science: "과학 원리, 과학 실험, 과학 이론, 과학 발견과 관련된 롱폼 비디오 주제",
    horror: "공포 이야기, 무서운 이야기, 미스터리, 초자연적 현상과 관련된 롱폼 비디오 주제",
    religion: "종교 이야기, 종교 철학, 종교 인물, 종교 역사와 관련된 롱폼 비디오 주제",
    domestic_story: "국내 이야기, 한국 이야기, 우리나라 이야기와 관련된 롱폼 비디오 주제",
    international_story: "해외 이야기, 세계 각국 이야기, 국제 이슈와 관련된 롱폼 비디오 주제",
    economy: "경제 이야기, 경제 이론, 경제 사건, 경제 인물과 관련된 롱폼 비디오 주제",
    war: "전쟁 이야기, 전쟁 역사, 전쟁 인물, 전쟁 전략과 관련된 롱폼 비디오 주제",
    affair: "사건 사고, 역사적 사건, 중요한 사건과 관련된 롱폼 비디오 주제",
    ancient: "고대 이야기, 고대 문명, 고대 역사와 관련된 롱폼 비디오 주제",
    biology: "생물학, 생명 과학, 생물 이야기와 관련된 롱폼 비디오 주제",
    greek_roman_mythology: "그리스 로마 신화, 신화 이야기, 신화 인물과 관련된 롱폼 비디오 주제",
    death: "죽음, 사망, 장례, 종교적 죽음과 관련된 롱폼 비디오 주제",
    ai: "인공지능, AI 기술, AI 미래, AI 이야기와 관련된 롱폼 비디오 주제",
    alien: "외계인, UFO, 외계 생명체, 우주 생명체와 관련된 롱폼 비디오 주제",
    palmistry: "손금 보기, 손상학과 관련된 롱폼 비디오 주제",
    physiognomy: "관상 보기, 얼굴 상학과 관련된 롱폼 비디오 주제",
    fortune_telling: "점술, 운세 보기, 미래 예측과 관련된 롱폼 비디오 주제",
    urban_legend: "도시 전설, 괴담, 소문과 관련된 롱폼 비디오 주제",
    serial_crime: "연쇄 범죄, 범죄 사건, 범죄자 이야기와 관련된 롱폼 비디오 주제",
    unsolved_case: "미제 사건, 미해결 사건, 수수께끼 사건과 관련된 롱폼 비디오 주제",
...
export async function generateScriptPlan(
  ...
  // 타입이 없으면 에러를 발생시키지 않고, 호출하는 쪽에서 AI 추천을 받아서 전달하도록 함
  // 하지만 호출하는 쪽에서 처리하지 못한 경우를 대비해 기본값 사용
  if (!contentType) {
    console.warn("[generateScriptPlan] ⚠️ contentType이 없습니다. 호출하는 쪽에서 AI 추천을 받아야 합니다.")
    throw new Error("콘텐츠 타입이 필요합니다. 콘텐츠 타입을 선택하거나, AI 자동 판단 기능을 사용해주세요.")
  }
  
  const finalContentType = contentType
  console.log("[generateScriptPlan] ✅ 최종 contentType:", finalContentType)

  // 커스텀 타입 선택 시 설명이 필수입니다.
  if (finalContentType === "custom" && (!customContentTypeDescription || !customContentTypeDescription.trim())) {
...
- 오직 기획안만 출력하십시오. 추가 설명이나 메타 정보는 포함하지 마십시오.`
  ...
  try {
    ...
    console.log("[generateScriptPlan] finalContentType 값:", finalContentType)
    console.log("[generateScriptPlan] finalContentType 타입:", typeof finalContentType)
    console.log("[generateScriptPlan] finalContentType === 'A':", finalContentType === "A")
    console.log("[generateScriptPlan] finalContentType === 'B':", finalContentType === "B")
    console.log("[generateScriptPlan] finalContentType === 'C':", finalContentType === "C")
    console.log("[generateScriptPlan] finalContentType === 'D':", finalContentType === "D")
    console.log("[generateScriptPlan] finalContentType === 'custom':", finalContentType === "custom")
    
    let systemPrompt = ""
    let userPrompt = ""

    if (finalContentType === "A") {
...
✅ 올바른 형식: "옛날 옛날에, 조선이라는 나라에 세종대왕이라는 지혜로운 왕이 살았습니다. 그 왕은 백성들을 사랑하는 마음이 깊었는데..."`
    ...
    } else if (finalContentType === "custom" && customContentTypeDescription) {
...
${category === "folktale" ? `⚠️ 레퍼런스 대본의 이야기 형식, 톤, 구조를 정확히 따르세요. 레퍼런스 대본이 이야기 형식이면, 기획안도 반드시 이야기 형식으로 작성하세요.` : "위 레퍼런스 대본을 참고하여 기획안을 작성하세요."}`
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (finalContentType && finalContentType !== "custom") {
          const expectedType = `Type ${finalContentType}`
          const expectedTypeLetter = finalContentType
...
export async function generateScriptDraft(
  ...
  try {
    ...
    const finalContentTypeForDraft = contentType || "A"
    const shouldUseStoryMode = isStoryMode || category === "folktale" || finalContentTypeForDraft === "D"
    
    if (shouldUseStoryMode) {
      // 스토리형 프롬프트
      systemPrompt = `*(2) 대본 작성
${category === "folktale" || finalContentTypeForDraft === "D" ? `
⚠️⚠️⚠️ 매우 중요: ${category === "folktale" ? '이 대본은 "옛날이야기(민담)" 카테고리입니다.' : finalContentTypeForDraft === "D" ? '사용자가 Type D (이야기형)를 선택했습니다. 이 대본은 "옛날이야기(민담)" 형식으로 작성되어야 합니다.' : ''}
