...
export async function generateImageWithNanobanana(
  ...
  /** info-shopping 등 고품질이 필요할 때 nano-banana-pro 사용 */
  preferPro?: boolean
): Promise<string> {
...
- 제품만 보여주고 손이나 사람은 포함하지 않음 (product only, no hands, no people)`}`
    ...
    // google/nano-banana 또는 nano-banana-pro
    // 9:16 비율로 이미지 생성
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        response = await fetch(`https://api.replicate.com/v1/models/${modelPath}/predictions`, {
      method: "POST",
      headers: {
...
/** 썰 채널형 스토리 쇼핑 대본: 훅 제목 + 나레이션 라인 배열 */
export interface SsulStoryScriptResult {
  hookTitle: string
  channelLabel: string
  bodyLines: string[]
  /** 장면용 짧은 나레이션 (bodyLines와 동일하거나 쪼갠 버전) */
  scenes: Array<{ narration: string }>
  /** TTS/기존 파이프용 이어붙인 대본 */
  fullScript: string
}

export async function generateSsulStoryScript(
  productName: string,
  productDescription: string,
  apiKey?: string,
  sceneCount = 6,
  duration: 12 | 15 | 20 | 30 = 15
): Promise<SsulStoryScriptResult> {
  const GPT_API_KEY = apiKey || process.env.GPT_API_KEY || process.env.OPENAI_API_KEY || process.env.CHATGPT_API_KEY
  if (!GPT_API_KEY) {
    throw new Error("GPT API 키가 설정되지 않았습니다. 환경 변수를 확인해주세요.")
  }
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `당신은 한국 유튜브 '썰/스토리' 채널 스타일의 쇼핑 숏폼 작가입니다.
영상은 상단에 훅 제목, 아래에 한 줄씩 나레이션이 바뀌고, 하단에는 제품 사진/영상이 나옵니다.
인물 리뷰어 톤이 아니라, 커뮤니티 글·썰 톤으로 호기심을 자극하세요.

반드시 JSON만 출력:
{
  "hookTitle": "클릭을 유도하는 짧은 훅 제목 (15자 내외, 제품명 암시 가능)",
  "channelLabel": "방구석",
  "bodyLines": ["나레이션1", "나레이션2", ...],
  "scenes": [{"narration": "장면별 한 문장"}, ...]
}

규칙:
- bodyLines / scenes 길이는 약 ${sceneCount}개
- 각 narration은 한 문장, 공백 포함 20~45자
- 직접 구매 강요·링크 CTA 금지
- 제품의 반전/문제해결/의외성이 드러나게
- 전체 호흡은 약 ${duration}초 쇼츠에 맞춤`,
        },
        {
          role: "user",
  ...
  if (!response.ok) {
    ...
    throw new Error(`썰 대본 생성 실패: ${response.status} ${err.slice(0, 200)}`)
  }

  const data = await response.json()
  const raw = data.choices?.[0]?.message?.content || "{}"
  let parsed: Partial<SsulStoryScriptResult> = {}
  try {
    parsed = JSON.parse(raw)
  } catch {
    throw new Error("썰 대본 JSON 파싱 실패")
  }

  const hookTitle = (parsed.hookTitle || `${productName} 썰`).trim()
  const channelLabel = (parsed.channelLabel || "방구석").trim()
  const bodyLines = Array.isArray(parsed.bodyLines)
    ? parsed.bodyLines.map((s) => String(s).trim()).filter(Boolean)
    : []
  let scenes = Array.isArray(parsed.scenes)
    ? parsed.scenes
        .map((s) => ({ narration: String((s as { narration?: string }).narration || "").trim() }))
        .filter((s) => s.narration)
    : bodyLines.map((narration) => ({ narration }))

  if (scenes.length === 0) {
    scenes = [{ narration: `${productName}, 이거 진짜 반전이었어요` }]
  }
  ...
  return {
    hookTitle,
    channelLabel,
    bodyLines: bodyLines.length ? bodyLines : scenes.map((s) => s.narration),
    scenes,
    fullScript,
  }
}
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `당신은 유튜브 쇼츠·스토리형 쇼핑 숏폼 메타데이터 작성 전문가입니다.
광고처럼 보이지 않는 이야기(썰) 영상용 제목, 설명, 태그를 만들어 주세요.

규칙:
1. 제목: 50자 이내, 호기심·반전·공감을 자극하는 후킹 문구. 노골적 광고 문구는 피하고, 이모지 사용 가능
2. 설명: 500자 이내, 스토리 요약 + 제품이 자연스럽게 등장하는 포인트. 링크/해시태그 공간 확보
3. 태그: 10-15개, 제품명·카테고리·스토리 키워드·쇼츠 검색어 포함
...
위 정보를 바탕으로 유튜브 쇼츠 업로드용 제목, 설명, 태그를 생성해주세요.`,
          },
        ],
        max_tokens: 500,
        temperature: 0.8,
