...
export interface AnimalCoupangProductSelection {
  productId: string
  productName: string
  productPrice: number
  productImage: string
...
export interface ShoppingProjectData {
  ...
  coupangUrl?: string
  selectedCoupangProduct?: AnimalCoupangProductSelection
  coupangSearchQuery?: string
  ...
  // 진행 상태 (video/render/thumbnail = 레거시)
  activeStep?:
    | "product"
    | "script"
    | "voice"
  ...
  /** 새 동물 쇼핑 스튜디오 brief (단계형 UI) */
  animalBrief?: AnimalShoppingBrief
  ...
  try {
    ...
    if (updates.data !== undefined) {
      ...
      try {
        ...
        updateData.data = { ...updates.data, appVariant: "animal" }
