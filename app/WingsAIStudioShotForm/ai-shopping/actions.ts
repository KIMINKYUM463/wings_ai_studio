...
// Replicate 이미지 모델을 사용한 이미지 생성 (제품 이미지 참고)
function normalizeProductImageRefs(
  productImageBase64: string | string[] | undefined
): string[] {
  const raw = Array.isArray(productImageBase64)
    ? productImageBase64
    : productImageBase64
      ? [productImageBase64]
      : []
  ...
  for (const item of raw) {
    ...
    if (trimmed.startsWith("blob:")) {
      console.warn("[Shopping] blob 제품 이미지는 Replicate 입력에서 제외합니다.")
      continue
    }
    if (
      ...
      out.push(`data:${mimeType};base64,${trimmed}`)
    }
...
export async function generateImageWithNanobanana(
  ...
  try {
    const normalizedSceneIndex = (sceneIndex === undefined
      ? 0
      : Math.abs(Math.trunc(sceneIndex)) % 3) as 0 | 1 | 2

    // 이미지 생성 프롬프트 생성
    // 제품 본체(색·형태·프린트/로고)는 보존하고, 장면 위 자막·워터마크만 금지
    ...
    try {
      if (referenceMode === "strict-review") {
        // 리뷰 사진 보존 모드는 프롬프트로 손·신체 구도를 그대로 유지합니다.
        needsHands = false
      } else {
      // OpenAI API 키: 클라이언트(localStorage) → 서버 env
      const openaiApiKey =
        openaiApiKeyFromClient?.trim() || process.env.OPENAI_API_KEY
      if (openaiApiKey) {
    ...
    if (sceneScript && sceneScript.trim() && sceneScript.length > 50) {
      ...
      const config = sceneConfigs[normalizedSceneIndex] || sceneConfigs[0]
...
품질: 고품질, 전문적인 제품 촬영, 매력적인 구도, 9:16 세로 비율, vertical composition.`
    ...
    if (productRefs.length > 0) {
      imagePrompt =
        referenceMode === "strict-review"
          ? `${imagePrompt}

GUIDED REVIEW PHOTO TRANSFORM:
- Treat the attached review image as a product identity and usage-context reference, not as a composition or pose template.
- Preserve the exact product model, color, material, proportions, function, and any logos/lettering/prints that belong to the product itself.
- If a hand, arm, or body is visible, keep a person using the product, but deliberately change the hand gesture, grip, finger placement, product orientation, camera angle, framing, and subject position.
- If the product is wrapped around, worn on, held by, or touching a person's body, preserve that usage meaning while restaging it with a new natural pose.
- Do not convert a hand-held scene into a product-only still life. Do not add or remove product parts.
- TRANSFORM MUST BE CLEARLY VISIBLE: redesign the composition, pose, background, lighting, color grading, depth of field, and photographic polish.
- Do not return a near-copy of the input image. Produce a newly staged commercial-quality 9:16 photograph where only product identity and usage meaning remain consistent.`
          : `${imagePrompt}

CRITICAL PRODUCT IDENTITY (HIGHEST PRIORITY):
- The attached image(s) are the REAL product to sell. Recreate THAT exact item — not a similar or generic substitute.
- Keep exact color, silhouette, materials, stitching, hardware, and proportions from the reference.
- KEEP all product-native branding: printed logos, lettering, embroidery, graphic prints, chest/back text, patterns (e.g. brand names on apparel). Do NOT erase, blank out, invent, or replace them.
...
${personIdentityLockPrompt({ hasCharacterReference: characterRefs.length > 0 })}`
    ...
    // 최종 제약을 프롬프트 맨 끝에 두어 가장 높은 우선순위로 적용한다.
    imagePrompt = `${imagePrompt}

FINAL OUTPUT CONSTRAINTS — THESE OVERRIDE ANY CONFLICTING INSTRUCTION ABOVE:
- Create a true full-bleed 9:16 vertical photograph that fills the entire canvas edge-to-edge.
- ${
      referenceMode === "strict-review"
        ? "Preserve product identity (including on-product logos/prints) and usage meaning, but visibly change the hand pose, grip, composition, camera angle, framing, background, and photographic treatment for 9:16."
        : "Recompose and crop the scene naturally for 9:16. Extend the photographic background all the way to every edge."
    }
- NO white space, black bars, letterboxing, pillarboxing, blank bands, frames, borders, margins, padding, poster layout, card layout, split layout, or text panel.
- FORBIDDEN overlays only: floating captions, subtitles, watermarks, UI chrome, price tags, stickers, or any text added on top of the photo that is NOT part of the physical product.
...
- Output only one continuous photorealistic scene of the SAME product as the reference.`
      ...
      try {
        ...
        response = await fetch(
            ...
            : "https://api.replicate.com/v1/models/google/nano-banana/predictions",
          {
            ...
            body: JSON.stringify({
              ...(isQwenImage && {
                // img2img 입력을 지원하는 qwen/qwen-image 버전
                version: "905e345fe1dfe10d628daac2140dd8dea471c0d99793ef0fdc46a15c688b62fb",
              }),
              input: {
                prompt: imagePrompt,
        ...
        console.error("[Shopping] 이미지 생성 오류:", errorText.substring(0, 300))
        lastError = new Error(
          `이미지 생성 실패: ${response.status} - ${errorText.substring(0, 200)}`
        )

        // 503은 서비스 전체 장애인 경우가 많아 1회만 재시도하고 빠르게 폴백
        const isHardOutage = response.status === 503
        const retriable =
          response.status === 500 ||
          response.status === 502 ||
          response.status === 503 ||
          response.status === 504
        const retryBudget = isHardOutage ? 1 : maxRetries - 1
        if (retriable && attempt < retryBudget) {
          console.warn(
            `[Shopping] 서버 오류 (${response.status}), 재시도 예정:`,
        ...
        // 이미 위에서 throw한 API 오류는 재시도하지 않음 (네트워크만 재시도)
        if (error === lastError && /이미지 생성 실패:/.test(lastError.message)) {
          throw lastError
        }

        // 네트워크 오류나 타임아웃도 재시도
        if (attempt < maxRetries - 1 && error instanceof TypeError) {
          console.warn(`[Shopping] 네트워크 오류, 재시도 예정:`, error.message || error)
          continue
        }
    ...
    if (data.status === "succeeded" && data.output) {
      ...
      console.log(`[Shopping] ${modelLabel} 이미지 생성 완료:`, imageUrl)
      return imageUrl
      ...
      while (attempts < maxAttempts) {
        ...
        if (statusData.status === "succeeded" && statusData.output) {
          ...
          console.log(`[Shopping] ${modelLabel} 이미지 생성 완료 (폴링):`, imageUrl)
          return imageUrl
    ...
    console.error(`[Shopping] ${modelLabel} 이미지 생성 실패:`, error)
    throw error
  }
}
...
export type PixabayKeywordSuggestion = {
  labelKo: string
  queryEn: string
}

/** 장면 대사에서 사용자가 고를 Pixabay 검색 키워드 후보를 생성 */
export async function generatePixabayKeywordSuggestions(
  narration: string,
  productName: string,
  apiKey?: string
): Promise<PixabayKeywordSuggestion[]> {
  const openaiApiKey =
    apiKey || process.env.OPENAI_API_KEY || process.env.GPT_API_KEY || process.env.CHATGPT_API_KEY
  if (!openaiApiKey) {
    throw new Error("키워드 추천을 위해 OpenAI API 키가 필요합니다.")
  }
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `쇼핑 숏폼의 한 장면에 어울리는 Pixabay 사진 검색어 후보를 만드세요.
JSON만 출력: {"suggestions":[{"labelKo":"한국어 키워드","queryEn":"English search query"}]}
규칙:
- 정확히 5개
- labelKo는 사용자가 이해하기 쉬운 한국어 2~5단어
- queryEn은 Pixabay 검색에 적합한 자연스러운 영어 2~5단어
- 서로 다른 배경·상황·분위기를 제안
- 인물 얼굴, 로고, 상표명, 글자 이미지는 피함`,
        },
        {
          role: "user",
          content: `제품명: ${productName || "(없음)"}\n장면 대사: ${narration}`,
        },
      ],
    }),
  })
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => "")
    throw new Error(`키워드 추천 실패: ${response.status} ${errorText.slice(0, 120)}`)
  }

  const data = await response.json()
  const content = data.choices?.[0]?.message?.content
  const parsed = content ? JSON.parse(content) : {}
  const suggestions: unknown[] = Array.isArray(parsed.suggestions) ? parsed.suggestions : []
  return suggestions
    .filter(
      (item: unknown): item is PixabayKeywordSuggestion =>
        typeof item === "object" &&
        item !== null &&
        typeof (item as PixabayKeywordSuggestion).labelKo === "string" &&
        typeof (item as PixabayKeywordSuggestion).queryEn === "string"
    )
    .map((item) => ({
      labelKo: item.labelKo.trim(),
      queryEn: item.queryEn.trim(),
    }))
    .filter((item) => item.labelKo && item.queryEn)
    .slice(0, 5)
}

/** 이미지/영상 생성 프롬프트를 한글·영어 쌍으로 맞춤 */
export async function ensureBilingualPrompt(
  prompt: string,
  apiKey?: string
): Promise<{ ko: string; en: string }> {
  const input = prompt.trim()
  if (!input) return { ko: "", en: "" }
  ...
  if (!openaiApiKey) {
    throw new Error("한·영 프롬프트 변환을 위해 OpenAI API 키가 필요합니다.")
  }
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `You convert image/video generation prompts into a Korean + English pair.
Return JSON only: {"ko":"...","en":"..."}
Rules:
- Preserve visual meaning, product identity, composition, lighting, and constraints.
- ko: natural Korean prompt (same meaning).
- en: natural English prompt suitable for image/video models (same meaning).
- Do not add new creative directions that were not in the source.
- If the source is already one language, keep it faithful and translate the other.`,
        },
        {
          role: "user",
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => "")
    throw new Error(`프롬프트 한·영 변환 실패: ${response.status} ${errorText.slice(0, 120)}`)
  }
  ...
  if (!ko && !en) throw new Error("한·영 프롬프트를 만들지 못했습니다.")

  const hasHangul = /[가-힣]/.test(input)
  return {
    ko: ko || (hasHangul ? input : ""),
    en: en || (!hasHangul ? input : ""),
  }
}

/** 사용자가 입력한 한국어 무료 이미지·영상 검색어를 Pixabay용 영어로 변환 */
export async function translateFreeAssetSearchQuery(
  queryKo: string,
  sceneContext: string,
  apiKey?: string
): Promise<string> {
  const input = queryKo.trim()
  if (!input) return ""
  ...
  const openaiApiKey =
    ...
    process.env.CHATGPT_API_KEY
  if (!openaiApiKey) {
    throw new Error("한국어 검색어 자동 번역을 위해 OpenAI API 키가 필요합니다.")
  }
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `한국어 검색어를 Pixabay 무료 이미지·영상 검색에 적합한 영어 검색어로 변환하세요.
JSON만 출력: {"queryEn":"English search query"}
규칙:
- 핵심 사물·행동·장소·감정을 보존
- 자연스러운 영어 2~6단어
- 브랜드명, 인물 이름, 불필요한 수식어 제외
- 설명하거나 후보를 여러 개 만들지 말 것`,
        },
        {
          role: "user",
          content: `한국어 검색어: ${input}\n장면 맥락: ${sceneContext || "(없음)"}`,
        },
      ],
    }),
  })
  if (!response.ok) {
    const errorText = await response.text().catch(() => "")
    throw new Error(`검색어 번역 실패: ${response.status} ${errorText.slice(0, 120)}`)
  }
  const data = await response.json()
  const content = data.choices?.[0]?.message?.content
  const parsed = content ? JSON.parse(content) : {}
  const translated = typeof parsed.queryEn === "string" ? parsed.queryEn.trim() : ""
  if (!translated) throw new Error("영어 검색어를 만들지 못했습니다.")
  return translated
}

export type CnVideoKeywordSuggestion = {
  labelKo: string
  reason: string
}

/** 도우인·샤오홍슈 영상 검색 전, 제품 중심의 한국어 검색어 추천 */
export async function generateCnVideoKeywordSuggestions(
  productName: string,
  productDescription: string,
  platform: "douyin" | "xiaohongshu",
  apiKey?: string
): Promise<CnVideoKeywordSuggestion[]> {
  const openaiApiKey =
    apiKey ||
    process.env.OPENAI_API_KEY ||
    process.env.GPT_API_KEY ||
    process.env.CHATGPT_API_KEY
  if (!openaiApiKey) {
    throw new Error("검색어 추천을 위해 OpenAI API 키가 필요합니다.")
  }

  const platformName = platform === "douyin" ? "도우인" : "샤오홍슈"
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    method: "POST",
    headers: {
      Authorization: `Bearer ${openaiApiKey}`,
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `당신은 중국 숏폼 플랫폼 ${platformName}의 영상 검색 전문가입니다.
한국 사용자가 이해하기 쉬운 한국어 검색어를 먼저 추천하세요.
JSON만 출력: {"suggestions":[{"labelKo":"검색어","reason":"추천 이유"}]}

규칙:
- 정확히 6개
- labelKo는 중국어로 번역했을 때 제품 자체를 찾기 좋은 구체적인 한국어 2~6단어
- 모든 추천어에 입력 정보에서 확인되는 제품명 또는 핵심 제품 카테고리를 반드시 포함
- 제품명, 제품 카테고리 동의어, 브랜드·모델, 형태, 재질, 용량, 핵심 사양처럼 제품을 식별하는 명사형 키워드만 사용
- "사용법", "사용하는 법", "전후 비교", "리뷰", "후기", "추천", "언박싱", "꿀팁", "장면", "영상" 같은 콘텐츠 의도 표현은 절대 포함하지 않음
- 입력에 없는 브랜드, 모델, 사양, 효능을 추측하거나 만들어내지 않음
- 상품명을 그대로 반복한 유사 문구 대신 검색 가능한 제품 카테고리 조합을 서로 다르게 제안`,
        },
        {
          role: "user",
          content: `상품명: ${productName || "(없음)"}\n제품 설명: ${productDescription || "(없음)"}`,
        },
      ],
    }),
  })
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => "")
    throw new Error(`검색어 추천 실패: ${response.status} ${errorText.slice(0, 120)}`)
  }
  ...
  const contentIntentPattern =
    /사용\s*법|사용하는\s*법|전후\s*비교|비포\s*애프터|리뷰|후기|추천|언박싱|꿀팁|장면|영상/
  return suggestions
    .filter(
      (item: unknown): item is CnVideoKeywordSuggestion =>
        typeof item === "object" &&
        item !== null &&
        typeof (item as CnVideoKeywordSuggestion).labelKo === "string" &&
        typeof (item as CnVideoKeywordSuggestion).reason === "string"
    )
    .map((item) => ({
      labelKo: item.labelKo.trim(),
      reason: item.reason.trim(),
    }))
    .filter((item) => item.labelKo && !contentIntentPattern.test(item.labelKo))
    .slice(0, 6)
}

/** Pixabay 무료 이미지 검색 (설정에 저장한 API 키 사용) */
export async function searchPixabayImages(
  query: string,
  apiKey?: string,
  options?: { page?: number; perPage?: number; orientation?: "all" | "horizontal" | "vertical" }
): Promise<{ hits: PixabayImageHit[]; total: number }> {
  const key = (apiKey || process.env.PIXABAY_API_KEY || "").trim()
  if (!key) {
    throw new Error("Pixabay API 키가 필요합니다. 설정에서 Pixabay API 키를 입력해주세요.")
  }
  ...
  const perPage = Math.min(Math.max(options?.perPage ?? 24, 3), 50)
  ...
  if (!response.ok) {
    ...
    console.error("[Shopping] Pixabay 검색 실패:", response.status, errText.slice(0, 200))
    throw new Error(`Pixabay 검색 실패: ${response.status}`)
  }
  ...
  const hits: PixabayImageHit[] = (data.hits || []).map((h) => ({
    id: h.id,
  ...
  return { hits, total: data.totalHits ?? hits.length }
}
...
/** Pixabay 무료 동영상 검색 (동일 API 키 사용) */
export async function searchPixabayVideos(
  query: string,
  apiKey?: string,
  options?: { page?: number; perPage?: number }
): Promise<{ hits: PixabayVideoHit[]; total: number }> {
  const key = (apiKey || process.env.PIXABAY_API_KEY || "").trim()
  if (!key) {
    throw new Error("Pixabay API 키가 필요합니다. 설정에서 Pixabay API 키를 입력해주세요.")
  }
  ...
  const page = options?.page ?? 1
  const perPage = Math.min(Math.max(options?.perPage ?? 20, 3), 50)
  ...
  if (!response.ok) {
    const errText = await response.text().catch(() => "")
    console.error("[Shopping] Pixabay 동영상 검색 실패:", response.status, errText.slice(0, 200))
    throw new Error(`Pixabay 동영상 검색 실패: ${response.status}`)
  }
  ...
  const hits: PixabayVideoHit[] = (data.hits || [])
    .map((h) => {
      ...
      const videoURL =
        videos.medium?.url || videos.large?.url || videos.small?.url || videos.tiny?.url || ""
      const previewURL =
...
export async function generateVideoPromptFromScript(
  ...
  prompt += `CRITICAL - PRODUCT VISIBILITY: The product must ALWAYS be fully visible in the frame throughout the entire video. The product must NEVER disappear, move out of frame, or become partially hidden. The product must stay in the center of the frame and remain fully visible from start to finish. `
  prompt += `CRITICAL - PRODUCT POSITION: The product must remain in the frame at all times. The product must not move out of the visible area. The product must stay within the frame boundaries throughout the entire video duration. `
  prompt += `ABSOLUTELY CRITICAL - PRODUCT SHAPE PRESERVATION: The product's shape, form, and structure must remain EXACTLY the same as the input image throughout the entire video. The product must NEVER be deformed, distorted, broken, cracked, bent, warped, stretched, or changed in any way. Even when hands are using or holding the product, the product must maintain its exact rigid form and physical integrity. Hands must NOT cause the product to deform, bend, or change shape. The product's physical structure must remain solid and unchanged at all times. `
...
async function generateProductActionPromptWithAI(
  ...
  if (!GPT_API_KEY) {
    // API 키가 없으면 기본 함수 사용
    return {
      ...getProductSpecificActions(productName, productDescription),
      needsHands: needsHandsInImage(productName, productDescription),
    }
  }
...
function getProductSpecificActions(productName: string, productDescription?: string): {
  ...
  const needsHands = needsHandsInImage(productName, productDescription)
  return {
    allowedActions: `ONLY the core/main function of ${productName} continuously throughout the entire video: demonstrate the product's main function continuously from start to finish, show the product being actively used for its intended purpose without interruption${needsHands ? ", hands operating the product naturally. CRITICAL: If hands are shown operating the product, hands must remain visible throughout the entire video from start to finish. Hands must NOT disappear mid-video. ABSOLUTELY CRITICAL - PRODUCT SHAPE PRESERVATION: Even when hands are using the product, the product's shape, form, and structure must remain EXACTLY the same as the input image. The product must NEVER be deformed, distorted, broken, cracked, bent, or warped by hands. Hands must NOT cause the product to change shape or deform. The product must maintain its exact rigid form and physical integrity at all times." : ", no hands, product only"}. Show ONLY the primary core action - the same action must continue throughout the video. ABSOLUTELY CRITICAL - PRODUCT SHAPE PRESERVATION: The product's shape, form, and structure must remain EXACTLY the same as the input image throughout the entire video. The product must NEVER be deformed, distorted, broken, cracked, bent, warped, or changed in any way.`,
...
- ABSOLUTELY CRITICAL - PRODUCT VISIBILITY (HIGHEST PRIORITY): The product must ALWAYS be fully visible in the frame throughout the entire video from start to finish. The product must NEVER disappear, move out of frame, become partially hidden, or leave the visible area. The product must stay within the frame boundaries at all times. The product must remain in the center or visible area of the frame throughout the entire video duration. DO NOT show the product disappearing or moving out of frame.
- ABSOLUTELY CRITICAL - PRODUCT SHAPE PRESERVATION (HIGHEST PRIORITY): The product's physical shape, form, structure, size, proportions, dimensions, design, colors, textures, and ALL visual features MUST remain EXACTLY the same as shown in the input image throughout the entire video. The product must NEVER be deformed, distorted, broken, cracked, bent, warped, stretched, shrunk, resized, modified, abstracted, stylized, or redesigned in ANY way.
- The product must be IDENTICAL to the reference image - same exact shape, same exact size, same exact proportions, same exact design, same exact colors, same exact structure
- ABSOLUTELY FORBIDDEN - NO PRODUCT DEFORMATION: DO NOT deform, distort, break, crack, bend, warp, stretch, shrink, resize, modify, abstract, stylize, or redesign the product in ANY way. The product's physical integrity must be maintained at all times.
- ABSOLUTELY FORBIDDEN - NO PRODUCT BREAKING: DO NOT show the product breaking, cracking, deforming, or losing its structural integrity. The product must maintain its exact physical form throughout the entire video.
- CRITICAL - PRODUCT FORM INTEGRITY: Even when hands are using or holding the product, the product's shape, form, and structure must remain EXACTLY the same as the input image. Hands must NOT cause the product to deform, bend, warp, or change shape. The product must maintain its rigid form and physical integrity even when being used or held.
- DO NOT change the product's form, structure, appearance, or colors - it must look EXACTLY like the input image
- The product's outline, edges, curves, angles, and all geometric features must remain unchanged and intact
...
- The product's materials, textures, and surface details must remain consistent and unchanged
- ABSOLUTELY CRITICAL - RIGID PRODUCT FORM: The product must maintain its rigid, solid form throughout the entire video. The product must NOT appear flexible, bendable, or deformable. The product's physical structure must remain solid and unchanged.
...
- CRITICAL: ${productActions.forbiddenActions}`
  ...
  if (imageIndex === 0) {
    ...
    prompt += `ABSOLUTELY CRITICAL - PRODUCT SHAPE PRESERVATION: The product shown in the input image must appear EXACTLY as it is - same shape, same size, same proportions, same design, same colors, same textures, same structure. DO NOT modify, deform, distort, break, crack, bend, warp, or change the product in any way. The product must be IDENTICAL to the input image. The product's physical form and structure must remain rigid and unchanged throughout the entire video. Even if hands are shown, the product must maintain its exact shape and form - hands must NOT cause the product to deform or change shape. `
    ...
    prompt += `ABSOLUTELY FORBIDDEN - NO OBJECTS ON PRODUCT: DO NOT place any other products, objects, items, or things on top of or above the product. The product must remain clear and unobstructed. DO NOT show anything being placed on the product, stacked on the product, or positioned above the product. The product surface must remain empty and clear throughout the entire video. `
    prompt += `ABSOLUTELY CRITICAL - PRODUCT VISIBILITY: The product must ALWAYS be fully visible in the frame throughout the entire video. The product must NEVER disappear, move out of frame, or become partially hidden. The product must stay in the center of the frame and remain fully visible from start to finish. The product must remain within the frame boundaries at all times. `
    prompt += `ABSOLUTELY CRITICAL - PRODUCT SHAPE PRESERVATION: The product's shape, form, and structure must remain EXACTLY the same as the input image throughout the entire video. The product must NEVER be deformed, distorted, broken, cracked, bent, warped, or changed in any way. The product's physical form and structure must remain rigid and unchanged. `
    ...
    prompt += `ABSOLUTELY FORBIDDEN - NO OBJECTS ON PRODUCT: DO NOT place any other products, objects, items, or things on top of or above the product. The product must remain clear and unobstructed. DO NOT show anything being placed on the product, stacked on the product, or positioned above the product. The product surface must remain empty and clear throughout the entire video. `
    prompt += `ABSOLUTELY CRITICAL - PRODUCT VISIBILITY: Even in a detail shot, the product must ALWAYS be visible in the frame throughout the entire video. The product must NEVER completely disappear or move completely out of frame. The product detail being shown must remain visible and within the frame boundaries at all times. `
    prompt += `ABSOLUTELY CRITICAL - PRODUCT SHAPE PRESERVATION: Even in a detail shot, the product's shape, form, and structure must remain EXACTLY the same as the input image throughout the entire video. The product must NEVER be deformed, distorted, broken, cracked, bent, warped, or changed in any way. The product detail being shown must maintain its exact form and structure. `
    prompt += `Emphasize product details, textures, materials, buttons, screens, design elements, or specific features. `
    prompt += `Product may be shown partially or in close-up - this is different from the full shot which shows the entire product. However, the product detail being shown must remain visible throughout the entire video. `
...
export type ShoppingImagePrompt = {
  type: string
  prompt: string
  description: string
  scriptText: string
...
- 제품에 인쇄·자수된 로고/레터링/그래픽 프린트(의류 가슴·등 글씨 등)는 반드시 그대로 유지. 지우거나 다른 로고로 바꾸지 말 것
- 비슷한 다른 상품·제네릭 대체품을 만들지 말 것 (same exact product as reference)
    ...
    return [
      {
        ...
        pixabayKeyword: "product full shot"
      },
      {
        type: "디테일 샷",
        prompt: `${productName} product, extreme close-up macro shot, very zoomed in detail shot, highlighting product features and details, product texture and material visible, only part of product visible in frame, no hands, no human hands, product only, isolated product, maintain exact original product buttons and features from reference image, no additional buttons or features, professional product photography, high quality, 9:16 aspect ratio`,
        ...
        pixabayKeyword: "product detail photo"
      },
      {
        type: "각도와 배경 다르게",
        ...
        pixabayKeyword: "product lifestyle photo"
      }
    ]
  }
}
...
위 제품을 분석하여 숏폼 영상 제작용 3장의 이미지 프롬프트를 생성해주세요.`,
    ...
    return [
      {
        type: "연출샷",
        prompt: `${productName} product, full product shot, clean background, photorealistic photograph, professional product photography, natural lighting, high quality, 9:16 aspect ratio`,
        description: "제품 전체 연출샷"
      },
      {
        type: "배경 변경",
        prompt: `${productName} product, different background, lifestyle setting, photorealistic photograph, professional product photography, natural lighting, high quality, 9:16 aspect ratio`,
        description: "다른 배경의 제품 사진"
      },
      {
        type: "디테일샷",
        prompt: `${productName} product, full product visible, entire product fills the frame, product details emphasized, highlighting product features, NOT close-up, NOT macro shot, entire product must be visible, photorealistic photograph, professional product photography, natural lighting, high quality, 9:16 aspect ratio`,
...
export async function generateImage(
  ...
  productName: string,
  replicateApiKey?: string,
  productImageBase64?: string | string[],
  productDescription?: string,
  apiKey?: string,
  ...
  if (!imagePrompts || imagePrompts.length === 0) {
    throw new Error("이미지 프롬프트가 1개 이상 필요합니다. 먼저 프롬프트를 생성해주세요.")
  }

  if (imagePrompts.some(({ prompt }) => !prompt?.trim())) {
    throw new Error("모든 이미지 프롬프트에 비어 있지 않은 prompt가 필요합니다.")
  }
...
export type ThumbnailPhraseTitleCandidate = {
...
/**
 * TFstudio generate-titles 대응 — 썸네일 2줄 문구 + 영상 제목 후보
 */
export async function generateThumbnailPhraseAndTitles(args: {
  productName: string
  script?: string
  productDescription?: string
  count?: number
  apiKey?: string
  /** TFstudio 템플릿 스택 thumbnail_focus */
  templateFocus?: string
  /** Codex styleVariant instructions */
  styleInstructions?: string
}): Promise<ThumbnailPhraseTitleCandidate[]> {
  const apiKey = args.apiKey || process.env.OPENAI_API_KEY || process.env.GPT_API_KEY
  if (!apiKey) {
    throw new Error("OpenAI API 키가 필요합니다. 설정에서 API 키를 입력해주세요.")
  }

  const count = Math.min(Math.max(args.count ?? 8, 3), 12)
  const scriptSlice = (args.script || "").trim().slice(0, 1800)
  const descSlice = (args.productDescription || "").trim().slice(0, 700)

  const system = `당신은 한국 쇼핑 숏폼 썸네일 카피·영상 제목 전문입니다.
반드시 JSON만 출력: {"items":[{"thumbnailText":"줄1 / 줄2","videoTitle":"...","line1":"...","line2":"..."}]}

규칙:
- line1·line2는 각 4~10자(가능하면 6~8자), 합치면 한 후킹
- thumbnailText는 "line1 / line2" 형식
- videoTitle은 35~50자 안팎, 제품 키워드+문제+궁금증
- 제목=문제/궁금증, 썸네일 문구=구체 행동/손해/반전 (같은 말 반복 금지)
- 금지: 무조건, 100%, 인생템, 완치, 치료, 보장, 대박, 레전드
- 광고체 금지. 사람이 궁금해서 누르는 톤
- items 정확히 ${count}개, 서로 다른 구조`

  const user = `제품명: ${args.productName}
제품 설명: ${descSlice || "(없음)"}
템플릿 focus: ${args.templateFocus || "(없음)"}
스타일 지시: ${args.styleInstructions || "(없음)"}
대본/SRT 근거:
${scriptSlice || "(없음)"}

위 근거만으로 썸네일 문구·영상 제목 후보를 만드세요. 템플릿·스타일 톤을 문구 각도에 반영하세요.`
  ...
  if (!response.ok) {
    ...
    throw new Error(`문구/제목 추천 실패: ${response.status} ${err.slice(0, 160)}`)
  }

  const data = await response.json()
  const raw = data.choices?.[0]?.message?.content || "{}"
  let parsed: { items?: Array<Partial<ThumbnailPhraseTitleCandidate>> } = {}
  try {
  ...
  const items = (parsed.items || [])
    .map((it) => {
      const line1 = (it.line1 || "").trim()
      const line2 = (it.line2 || "").trim()
      const thumbnailText =
        (it.thumbnailText || "").trim() ||
        (line1 && line2 ? `${line1} / ${line2}` : "")
      const videoTitle = (it.videoTitle || "").trim()
      if (!line1 || !line2 || !videoTitle) return null
      return { thumbnailText, videoTitle, line1, line2 }
    })
    .filter(Boolean) as ThumbnailPhraseTitleCandidate[]

  if (items.length === 0) {
    // fallback: 단일 후킹 생성 후 변형
    const hook = await generateThumbnailHookingTextLib(args.productName, apiKey)
    return [
      {
        line1: hook.line1,
        line2: hook.line2,
        thumbnailText: `${hook.line1} / ${hook.line2}`,
        videoTitle: `${args.productName} 때문에 고민이라면 꼭 보세요`.slice(0, 48),
      },
    ]
  }
...
export async function generateShortsThumbnail(
  ...
  styleHints?: { templateFocus?: string; styleInstructions?: string }
) {
  return generateShortsThumbnailLib(
    productName,
    replicateApiKey,
    productImageBase64,
    hookingText,
...
export type ShoppingVideoGenerationModel =
  | "seedance-1-pro-fast"
  | "seedance-1.5-pro"
  | "p-video-draft"

// AI 쇼핑에서 선택한 Replicate 이미지→영상 모델로 생성
export async function generateVideoWithSeedance(
  imageUrl: string,
  prompt: string, // 프롬프트 (필수)
  duration: number, // 영상 길이 (초 단위)
  replicateApiKey?: string,
  model: ShoppingVideoGenerationModel = "seedance-1-pro-fast"
): Promise<string> {
  const REPLICATE_API_TOKEN = replicateApiKey || process.env.REPLICATE_API_TOKEN
  ...
  const maximumDuration = model === "p-video-draft" ? 10 : 12
...
The main product remains anchored in the exact same position, scale, orientation, shape, count, color, texture, proportions, and visible features as the input image.
Treat the product as a physically solid object with stable geometry and identity. Preserve its silhouette, surface details, parts, edges, labels-free surfaces, and contact points frame by frame.
Use only subtle, physically plausible micro-motion already supported by the image, such as breathing, blinking, tiny natural hand movement, gentle fabric movement, or very slight ambient motion.
When a person is holding or touching the product, maintain the same hand-to-product contact and prevent slipping, teleporting, floating, sliding, rotating, resizing, morphing, duplication, disappearance, or spontaneous movement.
Do not invent unseen product parts, new functions, new objects, new people, or a different background. Do not change the number of products or people.
No cuts, transitions, reframing, zoom, pan, tilt, orbit, sudden motion, dramatic action, object replacement, deformation, or scene transformation.
The result should look like a real camera recorded the existing scene for ${duration} seconds with restrained, natural motion and strong temporal consistency.`

    // 사용자/장면 프롬프트 뒤에 안정성 계약을 배치해 상충하는 움직임 지시보다 우선 적용
