...
export default function YouTubeTrendsPage() {
  ...
  useEffect(() => {
    // 먼저 새로운 키 이름으로 확인
    let savedKey = localStorage.getItem("shotform_youtube_data_api_key")
    
    // 기존 키 이름으로 저장된 값이 있으면 마이그레이션
    if (!savedKey) {
      const oldKey = localStorage.getItem("youtube_api_key")
      if (oldKey) {
        localStorage.setItem("shotform_youtube_data_api_key", oldKey)
        savedKey = oldKey
      }
    }
  ...
  const handleSaveApiKey = () => {
    if (youtubeApiKey.trim()) {
      localStorage.setItem("shotform_youtube_data_api_key", youtubeApiKey.trim())
  ...
  const getApiKey = () => {
    ...
    return localStorage.getItem("shotform_youtube_data_api_key") || undefined
  }
