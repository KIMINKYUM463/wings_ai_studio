...
function proxyThumb(url: string): string {
  return xhsThumbProxy(url) || normalizeThumbUrl(url)
}
  ...
  return (
    <div
      className={cn(
        ...
        selected ? "border-violet-500 ring-1 ring-violet-500/50" : "border-white/10 hover:border-amber-500/40"
      )}
    >
      <div className="relative">
        ...
        />
        <span className="pointer-events-none absolute left-1.5 top-1.5 z-[1] rounded bg-black/75 px-1 py-0.5 text-[9px] text-amber-200">
          {item.videoUrl ? "▶ 영상" : "📷 노트"}
        </span>
        {badge ? (
...
export function DouyinActorBenchmarkPanel() {
  ...
  const loadStore = useCallback(async () => {
    ...
    setErr(null)
    try {
      const res = await fetch("/api/shotform/douyin-actor-benchmark?discover=1")
  ...
  const resolveSearchKeywords = useCallback(async (koInputs: string[]) => {
    return fetchCnKeywordTranslation({
      keywords: koInputs,
      openaiApiKey: shotformOpenAIKey(),
      platform: "douyin",
    })
  }, [])
  ...
  const runBenchmark = useCallback(async () => {
    ...
    setLoading(true)
    let keywordPairsResolved: KoZhKeywordPair[] = []
    let keywords: string[] = []
    ...
    try {
      for (let b = 0; b < totalBatches; b++) {
        ...
        const res = await fetch("/api/shotform/douyin-actor-benchmark", {
          method: "POST",
        ...
        if (!res.ok) {
          setErr(json.error || `배치 ${b + 1} 실패 (${res.status})`)
          break
        }
      ...
      setData({
        ...
        notice: `${lastNotice} · 검색어 ${keywords.join(" · ")} · 총 ${slugs.length} Actor · ${mergedResults.length}회 API 호출`,
      })
    } catch (e) {
  ...
  const estimatedCalls =
    selected.size *
    (massMode ? 1 : parseKoKeywordInputs(false, "", keywordsText).length)
  ...
  return (
    <div className="space-y-6">
      <StudioPageCard>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-amber-300/80">平台 · 抖音 · 선정 Actor {DOUYIN_VIDEO_APIFY_ACTORS.length}개</p>
            <p className="mt-1 text-xs text-slate-500">
              sian.agency · natanielsantos — 서비스·벤치마크 동일 목록
            </p>
          </div>
          <Button
            type="button"
      ...
      <StudioPageCard>
        ...
        <p className="mt-1 text-xs text-slate-500">
          한국어 키워드 입력 → GPT가 간체 中文로 변환 → 抖音 소스 검색 (OpenAI + 소스 검색 토큰 필요)
        </p>
        ...
        {translateLoading ? (
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {keywordPairs.map((p) => (
              <span
                key={`${p.ko}:${p.zh}`}
                className="rounded-lg border border-amber-500/25 bg-amber-950/20 px-2 py-1 text-xs text-amber-100"
              >
        ...
        <p className="mt-2 text-xs text-amber-400/90">
          예상 API 호출: <strong>{estimatedCalls}회</strong> · 배치 {DEFAULT_BATCH}개씩 · 선정 Actor만 비교
        </p>
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <Button type="button" className={studio.btnPrimary} disabled={loading || selected.size === 0} onClick={() => void runBenchmark()}>
            ...
            {loading ? "비교 중…" : `선택 ${selected.size}개 Actor 비교`}
          </Button>
        </div>
      ...
      {loading && !data ? (
        <StudioPageCard className="flex min-h-[120px] items-center justify-center gap-3 text-slate-400">
      ...
      {data ? (
        <>
          <StudioPageCard className="border-amber-500/20 bg-amber-950/10">
            ...
            {videoSummary.length === 0 ? (
              <p className="mt-4 py-6 text-center text-sm text-slate-500">
                영상을 가져온 Actor가 없습니다. 벤치마크를 다시 실행해주세요.
              </p>
            ) : (
          ...
          {allMergedVideos.length > 0 ? (
            <StudioPageCard className="border-amber-500/25 bg-amber-950/10">
              ...
              <p className="mt-1 text-xs text-slate-400">
                선택 Actor 병합 · 중복 URL 제거 · 키워드「{data.keywords.join(" · ")}」
              </p>
