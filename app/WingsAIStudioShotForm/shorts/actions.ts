...
export async function generateShortsScript(topic: string, duration: 15 | 30 | 60 | 120 | 180 = 60, apiKey?: string) {
  // 사용자가 제공한 API 키 사용 (없으면 환경 변수에서 가져오기)
  const GPT_API_KEY = apiKey || process.env.GPT_API_KEY || process.env.OPENAI_API_KEY || process.env.CHATGPT_API_KEY
  ...
  // 시간별 예상 문자 수 (초당 6.9자 기준, duration은 초 단위)
  const targetChars = Math.round(duration * 6.9)
  const durationSeconds = duration
  const durationMinutes = durationSeconds / 60
  const durationDisplay = durationSeconds < 60 ? `${durationSeconds}초` : `${durationMinutes}분`

  try {
    const systemPrompt = `당신은 유튜브 쇼츠 대본 전문가입니다. ${durationDisplay} 길이의 쇼츠에 최적화된 대본을 작성해주세요.
...
line1은 꾸며주는 말(후킹 문구)만, line2는 핵심 내용(주제 키워드 + 실제 내용)으로 작성해야 합니다.`
    ...
    if (jsonMatch) {
      ...
      if (!line1 || !line2) {
        ...
        // 더 강렬한 후킹 문구 사용
        line1 = line1 || `${firstWord}의 충격` || "99%가 모르는"
        line2 = line2 || "비밀 공개" || "지금 확인"
      }
      
      console.log("[Shorts] 제목 생성 완료:", { line1, line2 })
      
      return {
        line1: line1.trim(), // 길이 제한 제거
        line2: line2.trim(), // 길이 제한 제거
      }
    }
...
/**
 * 이미지 프롬프트 생성 함수 (간소화 버전 - shorts 전용)
 */
export async function generateImagePrompt(
  ...
  try {
    const categoryGuides: Record<string, string> = {
      ...
      psychology: "심리학, 인간 심리와 관련된 이미지. 깊이 있고 통찰력 있는 분위기.",
    }
...
프롬프트만 작성하고 설명은 추가하지 마세요.`
    ...
    console.error("[Shorts] 이미지 프롬프트 생성 실패:", error)
    throw error
  }
}
...
export async function generateImageWithReplicate(
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text()
      console.error("[Shorts] Replicate API 오류:", errorText)
      
      if (response.status === 404 || response.status === 422) {
        console.log("[Shorts] flux-pro 실패, stable-diffusion-xl 시도")
    ...
    if (data.status === "processing" || data.status === "starting" || data.status === "succeeded") {
      ...
      console.log("[Shorts] 이미지 생성 완료 (즉시):", imageUrl)
      return imageUrl
    } else {
      throw new Error(`이미지 생성 실패: ${data.error || "알 수 없는 오류"}`)
    }
  } catch (error) {
    console.error("[Shorts] Replicate 이미지 생성 실패:", error)
    throw error
  }
}
...
async function generateImageWithStableDiffusion(
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      ...
      if (statusData.status === "succeeded" && statusData.output) {
        const imageUrl = Array.isArray(statusData.output) 
          ? statusData.output[0] 
          : statusData.output
        console.log("[Shorts] Stable Diffusion 이미지 생성 완료:", imageUrl)
        return imageUrl
      } else if (statusData.status === "failed") {
        throw new Error(`이미지 생성 실패: ${statusData.error || "알 수 없는 오류"}`)
      }
    ...
    console.error("[Shorts] Stable Diffusion 이미지 생성 실패:", error)
    throw error
  }
}
